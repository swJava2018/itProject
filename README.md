...
## 가이드 순서  
1. 빌드된 결과물 테스트 > ■ 테스트 실행 가이드  
2. 개발환경 세팅 후, 테스트 > ■ 개발환경 설치 및 실행 가이드
...
### ■ 테스트 실행 가이드
#### 1.1. 프로젝트 및 실행가능한 빌드 결과물 다운 (git client 다운, git lfs 포함)
...
#### 2.1. STS(Spring Tool Suite 4) 다운
...
#### 2.3. 테스트
      1. 위의 [1.4 테스트 시나리오] 항목 참고
      ※ 주의, 개발 환경 세팅 전에 [■ 테스트 실행 가이드]를 따라하셨다면 PowerShell 창(실행 중인 서버) 종료한 후 시도해야합니다.
